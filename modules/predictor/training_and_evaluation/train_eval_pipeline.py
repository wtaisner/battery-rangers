...
def batch_train_and_eval(df: pd.DataFrame, df_name: str, folds: list, target: str, features: list, opt_type: Literal["grid_search", "bayesian_search"], save_dir: str) -> None:
    ...
    for m in models:
        ...
        train_fold = folds[0][0]
        train_df = df.iloc[train_fold]
        opt_folds = custom_data_split(train_df, target, 4, 4)

        model_name, model, param_grid = get_model(m)
        best_score, best_params = param_search(model, df, features, target, opt_folds, param_grid, opt_type)
        ...
        cv_eval(model, model_name, folds, df, features, target, df_name, os.path.join(save_dir, f"results_{model_name}.txt"), verbose=True)
