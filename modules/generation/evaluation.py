...
class MoleculeGenerationEvaluator:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.molecule_filter = MoleculeFilter(
            filters=None,
            molecule_type=kwargs.get("molecule_type", MoleculeType.SUBSTRATE),
            db_file=kwargs.get("db_file", "modules/bionemo/data/mol_db/substrate_properties.db"),
        )
        ...
        (
            self.valid_unique_generated_mols,
            self.valid_unique_generated_smiles_canon,
            self.n_generated_valid,
        ) = self._preprocess_smiles_list(self.generated_smiles, "Generated")
    ...
    def _preprocess_smiles_list(smiles_list: list[str], list_name: str, n_jobs: int = 10) -> tuple[list[Chem.Mol], list[str], int]:
        ...
        with mp.Pool(processes=n_jobs) as pool:
            ...
            results = list(
                tqdm(
                    pool.imap(_worker_preprocess_smiles, smiles_list),
                    total=len(smiles_list),
                    desc=f"Processing {list_name} SMILES",
                )
            )
    ...
    def evaluate(
        ...
        project: str = "molecule-generation",
            ...
            if log_examples:
                ...
                if num_novel == 0:
                    ...
                    selected_examples = np.random.choice(
                        list(novel_molecules_set),
                        size=min(num_examples, num_novel),
                        replace=False,
                    )
    ...
    def calculate_internal_diversity(self, p: int = 1, fp_radius: int = 2, fp_bits: int = 1024) -> float:
        ...
        if len(unique_valid_mols) < 2:
            logger.warning(
                "Cannot calculate internal diversity: requires at least 2 unique valid molecules, but found %d.",
        ...
        if num_fingerprints < 2:
            logger.warning(
                "Could not generate enough valid fingerprints (%d) for diversity calculation.",
                num_fingerprints,
            )
            return 0.0
        ...
        for i, j in tqdm(
            combinations(range(num_fingerprints), 2),
            desc="Calculating internal diversity",
        ):
            try:
